...
const MaqamSynth = () => {
  ...
  const reverbEffect = useRef(null);s
  ...
  const [reverbDecay, setReverbDecay] = useState(1.5); // New: Reverb Decay

  // --- Master Volume ---
  const [masterVolume, setMasterVolume] = useState(0.5); // New: Master Volume
  ...
  useEffect(() => {
    if (!synth.current) {
      ...
      gainNode.current = new Tone.Gain(masterVolume).toDestination(); // Initialize with masterVolume

      // Create effects
      delayEffect.current = new Tone.FeedbackDelay("8n", delayFeedback).set({ wet: 0 });
      reverbEffect.current = new Tone.Reverb({ decay: reverbDecay, wet: 0.05 }).set({ wet: 0 }); // Initialize with reverbDecay
  ...
  useEffect(() => {
    if (reverbEffect.current) {
      ...
      reverbEffect.current.decay = reverbDecay; // Set Reverb Decay
    }
  ...
  return (
    <div>
      <div className="controls">
        ...
        <div>
          <label htmlFor="reverb-decay-slider">Reverb Decay (s):</label>
          <input
